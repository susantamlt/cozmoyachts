...
class Yachts extends MX_Controller 
{
	public function __construct(){
		parent::__construct();
		$this->lang->load('data');
		$this->load->model('api/yachts_model','yachts');
	}
	...
	public function lists() {
		...
		if(isset($_hData['x-api-key']) && $_hData['x-api-key'] !=''){
			$apikey = $this->yachts->authonticationKey($_hData['x-api-key']);
			if($apikey > 0){
				$result = $this->yachts->listYachts($apikey);
				if(!empty($result)){
					$_data['status']='true';
					$_data['yachts']=$result;
				} else {
		...
		if(isset($_hData['x-api-key']) && $_hData['x-api-key'] !=''){
			...
			if($apikey > 0){
				...
				if ($this->form_validation->run() == FALSE){
					...
					$result = $this->yachts->viewYachts($data['pak_id']);
					if(!empty($result)){
						$_data['status']='true';
